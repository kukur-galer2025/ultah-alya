...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const breadEmojis = ['🍞', '🥐', '🥖', '🍰', '🎂'];
    const generatedBreads = [...Array(12)].map((_, i) => ({
      id: i,
      emoji: breadEmojis[Math.floor(Math.random() * breadEmojis.length)],
      left: `${Math.random() * 100}%`,
      top: `${Math.random() * 100}%`,
      duration: Math.random() * 4 + 3, // Diperlambat sedikit agar lebih smooth
  ...
  const handleNextMessage = () => {
    ...
    if (messageIndex < messageSequence.length - 1) {
      ...
      }, 1000); // Transisi dipercepat agar responsif
    }
  };
  ...
  return (
    <main className="min-h-[100dvh] w-full bg-[#11051b] text-white flex items-center justify-center p-4 md:p-8 overflow-hidden relative font-sans cursor-default selection:bg-pink-500/30">
      ...
      {/* BACKGROUND LAYER */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-purple-900/20 via-[#11051b] to-[#11051b] z-0 pointer-events-none transform-gpu" />
      ...
      {/* AMBIENT GLOWS (Dengan transform-gpu agar 60fps) */}
      <div className="pointer-events-none absolute top-0 right-0 w-[300px] h-[300px] bg-pink-600/10 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/3 z-0 transform-gpu" />
      <div className="pointer-events-none absolute bottom-0 left-0 w-[400px] h-[400px] bg-purple-600/10 rounded-full blur-[90px] translate-y-1/3 -translate-x-1/3 z-0 transform-gpu" />
      ...
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none">
        {stars.map((star) => (
          <motion.div
            ...
            style={{ width: star.width, height: star.height, left: star.left, top: star.top, willChange: "opacity" }}
      ...
      {/* BREADS LAYER (TYPO TELAH DIPERBAIKI -> pointer-events-none) */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        {breads.map((bread) => (
          <motion.div
            key={`bread-${bread.id}`}
            className="absolute transform-gpu"
            style={{ left: bread.left, top: bread.top, opacity: bread.opacity, fontSize: "1.2rem", willChange: "transform" }}
            animate={{ y: [0, -25, 0] }}
      ...
      {/* Menghapus filter: blur pada transisi agar animasi tidak patah-patah di HP */}
      <AnimatePresence mode="wait">
        ...
        {gameState === 'intro' && (
          <motion.div
            ...
            className="relative z-50 group flex flex-col items-center cursor-pointer p-6 md:p-12 text-center rounded-[2rem] hover:bg-white/[0.02] transition-colors duration-500"
            onClick={startExperience}
          >
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} className="relative transform-gpu">
              <Gift className="w-20 h-20 md:w-32 md:h-32 text-pink-300 drop-shadow-[0_0_15px_rgba(244,114,182,0.4)]" />
            </motion.div>
        ...
        {gameState === 'loading' && (
          <motion.div
            ...
            className="relative z-50 w-full max-w-sm md:max-w-lg bg-[#ffffff0a] border border-white/10 p-8 md:p-14 rounded-[2rem] shadow-2xl backdrop-blur-xl flex flex-col items-center justify-center min-h-[250px] md:min-h-[300px]"
          >
            <Stars className="w-8 h-8 md:w-10 md:h-10 text-pink-300 mb-6 md:mb-8 animate-pulse transform-gpu" />
        ...
        {gameState === 'typing' && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95 }}
            className="relative z-50 flex flex-col items-center justify-center min-h-[300px] w-full px-2"
          >
            <AnimatePresence mode="wait">
              <motion.div
                key={messageIndex}
                // ANIMASI LEBIH RINGAN (TANPA BLUR)
                initial={{ opacity: 0, y: 15, scale: 0.98 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, y: -15, scale: 0.98 }}
                transition={{ duration: 0.4, ease: "easeOut" }}
                className="flex flex-col items-center text-center w-full max-w-[95vw] md:max-w-3xl transform-gpu"
              >
                <div className="mb-6 md:mb-10 p-4 md:p-6 bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-full border border-pink-500/20 backdrop-blur-md shadow-lg">
            ...
            <motion.button
              ...
              // PERBESAR UKURAN TOMBOL UNTUK HP (px-8 py-4)
              className="relative z-50 mt-12 md:mt-16 flex items-center gap-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-4 md:px-10 md:py-5 rounded-full font-bold shadow-xl active:scale-95 transition-transform text-sm md:text-base tracking-wide"
            >
        ...
        {gameState === 'shatter' && (
          <motion.div key="shatter" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-[#fff5fa]">
            <motion.div animate={{ scale: [1, 50], opacity: [1, 0] }} transition={{ duration: 1.2, ease: "circIn" }} className="w-10 h-10 bg-white rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform-gpu" />
          </motion.div>
        )}
        ...
        {gameState === 'party' && (
          <motion.div
            ...
            className="relative z-50 flex flex-col items-center w-full max-w-[95vw] md:max-w-3xl"
          >
            {/* Blur dikurangi jadi backdrop-blur-xl agar enteng di HP */}
            <div className="bg-[#ffffff08] backdrop-blur-xl border border-white/10 p-6 sm:p-8 md:p-12 rounded-[2rem] w-full text-center shadow-2xl relative overflow-hidden group">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white via-pink-100 to-purple-400 mb-2 pb-2 leading-none">
                Happy Birthday
              </h1>
            ...
            <motion.button
              ...
              className="mt-8 md:mt-10 flex items-center gap-2 md:gap-3 bg-white/10 border border-white/20 backdrop-blur-lg text-white px-8 py-4 md:px-10 md:py-4 rounded-full font-medium shadow-xl active:scale-95 transition-all text-sm md:text-base"
            >
        ...
        {gameState === 'gallery' && (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="absolute inset-0 z-50 overflow-hidden flex items-center justify-center bg-[#0a0212]/95 backdrop-blur-lg"
          >
            ...
            {polaroids.map((photo, i) => (
              <motion.div
                ...
                // style willChange mengaktifkan hardware acceleration agar geser foto super mulus
                style={{ zIndex: i, touchAction: "none", willChange: "transform" }}
                className="absolute bg-[#fafafa] p-2 pb-10 md:p-3 md:pb-14 rounded-sm shadow-2xl cursor-grab active:cursor-grabbing transform-gpu"
                whileDrag={{ scale: 1.05, zIndex: 50, boxShadow: "0 30px 60px rgba(255,255,255,0.1)" }}
            ...
            <button
              ...
              className="absolute bottom-8 md:bottom-12 z-50 bg-white/20 backdrop-blur-md border border-white/30 text-white font-medium tracking-wider text-[10px] md:text-xs px-8 py-3 rounded-full active:bg-white/40 transition-all"
            >
              Tutup Album
            </button>
          </motion.div>
